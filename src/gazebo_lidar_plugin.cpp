...
#include "laser_gazebo_resources/gazebo_lidar_plugin.h"
...
void LidarPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) {
  ...
  newLaserScansConnection_ = parentSensor_->LaserShape()->ConnectNewLaserScans(boost::bind(&LidarPlugin::OnNewLaserScans, this));
  ...
  if (_sdf->HasElement("topic")) {
    ...
    gzwarn << "[gazebo_lidar_plugin]: " + names_splitted.front() + "::" + names_splitted.rbegin()[1] + " using lidar topic \"" << parentSensorModelName
           << "\"\n";
  }
  ...
  lidar_pub_ = node_handle_->Advertise<sensor_msgs::msgs::Range>("~/" + names_splitted[0] + lidar_topic_, 10);
}
...
void LidarPlugin::OnNewLaserScans() {
  ...
  if (signal_strength > low_signal_strength_) {
    signal_quality = static_cast<uint8_t>(99 * ((high_signal_strength_ - signal_strength) / (high_signal_strength_ - low_signal_strength_)) + 1);
  }
