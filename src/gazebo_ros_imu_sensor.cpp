...
#include <laser_gazebo_resources/gazebo_ros_imu_sensor.hpp>
...
void GazeboRosImuSensor::Load(gazebo::sensors::SensorPtr _sensor, sdf::ElementPtr _sdf) {
  ...
  if (!_sdf->HasElement("robot_namespace")) {
    RCLCPP_INFO(impl_->ros_node_->get_logger(), "Missing <robot_namespace>, defaults to \"uav1\"");
  ...
  if (!_sdf->HasElement("parent_frame_name")) {
    RCLCPP_INFO(impl_->ros_node_->get_logger(), "Missing <parent_frame_name>, defaults to \"sensor\"");
    parent_link_ = "sensor";
  } else {
    parent_link_ = _sdf->Get<std::string>("parent_frame_name");
  }
  ...
  impl_->pub_ = impl_->ros_node_->create_publisher<sensor_msgs::msg::Imu>("~/out", qos.get_publisher_qos("~/out", rclcpp::SensorDataQoS().reliable()));

  // Create message to be reused
  auto msg = std::make_shared<sensor_msgs::msg::Imu>();

  // Get frame for message
  msg->header.frame_id = robot_namespace_ + "/" + parent_link_ + "/imu";

  // Fill covariances
  // TODO(anyone): covariance for IMU's orientation once this is added to gazebo
  using SNT                              = gazebo::sensors::SensorNoiseType;
  msg->angular_velocity_covariance[0]    = gazebo_ros::NoiseVariance(impl_->sensor_->Noise(SNT::IMU_ANGVEL_X_NOISE_RADIANS_PER_S));
  msg->angular_velocity_covariance[4]    = gazebo_ros::NoiseVariance(impl_->sensor_->Noise(SNT::IMU_ANGVEL_Y_NOISE_RADIANS_PER_S));
  msg->angular_velocity_covariance[8]    = gazebo_ros::NoiseVariance(impl_->sensor_->Noise(SNT::IMU_ANGVEL_Z_NOISE_RADIANS_PER_S));
  msg->linear_acceleration_covariance[0] = gazebo_ros::NoiseVariance(impl_->sensor_->Noise(SNT::IMU_LINACC_X_NOISE_METERS_PER_S_SQR));
  msg->linear_acceleration_covariance[4] = gazebo_ros::NoiseVariance(impl_->sensor_->Noise(SNT::IMU_LINACC_Y_NOISE_METERS_PER_S_SQR));
  msg->linear_acceleration_covariance[8] = gazebo_ros::NoiseVariance(impl_->sensor_->Noise(SNT::IMU_LINACC_Z_NOISE_METERS_PER_S_SQR));

  impl_->msg_ = msg;

  impl_->sensor_update_event_ = impl_->sensor_->ConnectUpdated(std::bind(&GazeboRosImuSensorPrivate::OnUpdate, impl_.get()));
}
...
void GazeboRosImuSensorPrivate::OnUpdate() {
  ...
  msg_->angular_velocity    = gazebo_ros::Convert<geometry_msgs::msg::Vector3>(sensor_->AngularVelocity());
  msg_->linear_acceleration = gazebo_ros::Convert<geometry_msgs::msg::Vector3>(sensor_->LinearAcceleration());
#ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE_END();
