...
void GazeboRosP3D::Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf) {
  ...
  impl_->pub_        = impl_->ros_node_->create_publisher<nav_msgs::msg::Odometry>(impl_->topic_name_,
                                                                            qos.get_publisher_qos(impl_->topic_name_, rclcpp::SensorDataQoS().reliable()));
  ...
  if (!sdf->HasElement("local_twist")) {
    RCLCPP_WARN(impl_->ros_node_->get_logger(), "p3d plugin missing <local_twist>, defaults to false");
  ...
  if (impl_->frame_name_ != "/world" && impl_->frame_name_ != "world" && impl_->frame_name_ != "/map" && impl_->frame_name_ != "map") {
  ...
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(std::bind(&GazeboRosP3DPrivate::OnUpdate, impl_.get(), std::placeholders::_1));
}
