...
public:
  ...
  virtual void OnNewFrame(const rendering::CameraPtr camera, const transport::PublisherPtr publisher) {
    ...
    image->set_pixel_format(common::Image::ConvertPixelFormat(camera->ImageFormat()));
    image->set_step(camera->ImageWidth() * camera->ImageDepth());
    image->set_data(camera->ImageData(), camera->ImageDepth() * camera->ImageWidth() * camera->ImageHeight());
  ...
  virtual void OnNewDepthFrame(const rendering::CameraPtr camera, const transport::PublisherPtr publisher) {
    ...
    PublishGazeboDepthImage(publisher, depth_map_.data(), image_width, image_height, image_size);
  }
  ...
  virtual void OnNewDepthFrameRealistic(const rendering::CameraPtr camera, const transport::PublisherPtr publisher) {
    ...
    unsigned int y = 0U;
    for (unsigned int index = 0; index < image_size; ++index) {
      const float relative_x = 5.0F * static_cast<float>(x) / static_cast<float>(image_width);
      const float relative_y = 5.0F * static_cast<float>(y) / static_cast<float>(image_height);
      const float depth      = depth_data[index];

      const bool remove_pixel = IsDepthInvalid(depth) || perlin_noise_.noise(relative_x, relative_y, current_perlin_z) > perlin_empty_threshold_;

      if (remove_pixel) {
        small_depth_map_[index] = 0U;
      } else {
        const float noise_scale = depth > noise_min_distance_ ? depth : noise_min_distance_;
        const float noise       = noise_scale * normal_distribution_(random_generator_);
        float       noisy_depth = noise + depth;

        if (noisy_depth < 0.0F || noisy_depth > kDepthScaleMeters * std::numeric_limits<uint16_t>::max()) {
          noisy_depth = 0.0F;
        }
    ...
    cv::Mat depth_image(static_cast<int>(image_height), static_cast<int>(image_width), CV_16UC1, small_depth_map_.data());

    UpscaleDepthMap(depth_image, depth_image, static_cast<int>(scaling_));
    ApplyDepthBlur(depth_image);
    ApplyDepthErosion(depth_image);

    std::memcpy(depth_map_.data(), depth_image.ptr(0), sizeof(uint16_t) * output_size);

    PublishGazeboDepthImage(publisher, depth_map_.data(), output_width, output_height, output_size);
  }
...
private:
  static void UpscaleDepthMap(const cv::Mat &input, cv::Mat &output, int scale) {
    ...
    cv::Mat resized(static_cast<int>(output_rows), static_cast<int>(output_columns), CV_16UC1);
    ...
    for (unsigned int row = 0; row < output_rows; ++row) {
      for (unsigned int column = 0; column < output_columns; ++column) {
        *output_pixel = input_data[column / scale + (row / scale) * input_columns];
        ++output_pixel;
      }
    }
  ...
  void ConfigureSensorNames(const std::string &ros_namespace, const std::string &camera_name, const std::string &camera_suffix) {
    const std::string sensor_prefix = ros_namespace + "/" + camera_name + camera_suffix + "_";

    depth_camera_plugin_name_           = sensor_prefix + kDepthCameraSuffix;
    color_camera_plugin_name_           = sensor_prefix + kColorCameraSuffix;
    infrared_stereo_camera_plugin_name_ = sensor_prefix + kInfraredStereoCameraSuffix;
  ...
  void ConfigureRealisticDepth(sdf::ElementPtr sdf) {
    ...
    if (use_realistic_) {
      ...
      getSdfParam(sdf, "perlinEmptyThreshold", perlin_empty_threshold_, 0.8F);
      ...
      normal_distribution_    = std::normal_distribution<float>(0.0F, noise_per_meter_ / 3.0F);
  ...
  void AllocateDepthBuffers() {
    ...
    depth_map_.resize(scaling_ * image_width * scaling_ * image_height);
  ...
  void ConfigureGazeboTransport() {
    ...
    const std::string topic_root = "~/" + model_->GetName() + "/rs/stream/";

    // The additional separator is retained to preserve the original Gazebo
    // transport topic strings.
    depth_publisher_     = transport_node_->Advertise<msgs::ImageStamped>(topic_root + "/" + kDepthCameraTopic, 1, kDepthPublishFrequencyHz);
    infrared1_publisher_ = transport_node_->Advertise<msgs::ImageStamped>(topic_root + "/" + kInfrared1CameraTopic, 1, kInfrared1PublishFrequencyHz);
    infrared2_publisher_ = transport_node_->Advertise<msgs::ImageStamped>(topic_root + "/" + kInfrared2CameraTopic, 1, kInfrared2PublishFrequencyHz);
    color_publisher_     = transport_node_->Advertise<msgs::ImageStamped>(topic_root + "/" + kColorCameraTopic, 1, kColorPublishFrequencyHz);
  }
  ...
  bool IsDepthInvalid(float depth) const {
    return depth <= depth_camera_->NearClip() || depth >= depth_camera_->FarClip() || depth > kDepthScaleMeters * std::numeric_limits<uint16_t>::max() ||
           depth < 0.0F;
  }
  ...
  void ApplyDepthBlur(cv::Mat &depth_image) const {
    ...
    cv::GaussianBlur(depth_image, blurred_depth, cv::Size(blur_size_, blur_size_), 2 * blur_size_);
    cv::GaussianBlur(valid_mask, blurred_mask, cv::Size(blur_size_, blur_size_), 2 * blur_size_);
    cv::divide(blurred_depth, blurred_mask, blurred_depth, 255.0, CV_16UC1);
  ...
  void ApplyDepthErosion(cv::Mat &depth_image) const {
    ...
    const cv::Mat element = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(erosion_size_, erosion_size_));
    cv::erode(depth_image, depth_image, element);
  }

  void PublishGazeboDepthImage(const transport::PublisherPtr &publisher, const uint16_t *depth_data, unsigned int width, unsigned int height,
...
public:
  ...
  void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override {
    ...
    RCLCPP_INFO(ros_node_->get_logger(), "Realsense Gazebo ROS 2 plugin loading...");

    RealSensePlugin::Load(model, sdf);

    camera_name_                    = sdf->HasElement("camera_name") ? sdf->Get<std::string>("camera_name") : "rgbd";
    camera_suffix_                  = sdf->HasElement("camera_suffix") ? sdf->Get<std::string>("camera_suffix") : "";
    const std::string ros_namespace = sdf->HasElement("namespace") ? sdf->Get<std::string>("namespace") : "";
    ...
    static_tf_broadcaster_ = std::make_shared<tf2_ros::StaticTransformBroadcaster>(ros_node_);
  ...
  void OnNewFrame(const rendering::CameraPtr camera, const transport::PublisherPtr publisher) override {
    ...
    sensor_msgs::fillImage(image, pixel_format, camera->ImageHeight(), camera->ImageWidth(), camera->ImageDepth() * camera->ImageWidth(),
                           reinterpret_cast<const void *>(camera->ImageData()));

    const sensor_msgs::msg::CameraInfo camera_info = CreateCameraInfo(image, camera);
...
private:
  void ConfigureFrameIds(const std::string &ros_namespace) {
    const std::string frame_prefix = ros_namespace + "/" + camera_name_ + camera_suffix_ + "/";
    ...
    infrared1_camera_optical_frame_id_ = infrared1_camera_frame_id_ + "_optical";
    infrared2_camera_optical_frame_id_ = infrared2_camera_frame_id_ + "_optical";
  }

  void ConfigureParentTransform(sdf::ElementPtr sdf) {
    parent_frame_name_ = sdf->HasElement("parentFrameName") ? sdf->Get<std::string>("parentFrameName") : "world";

    x_     = sdf->HasElement("x") ? sdf->Get<double>("x") : 0.0;
    y_     = sdf->HasElement("y") ? sdf->Get<double>("y") : 0.0;
    z_     = sdf->HasElement("z") ? sdf->Get<double>("z") : 0.0;
    roll_  = sdf->HasElement("roll") ? sdf->Get<double>("roll") : 0.0;
    pitch_ = sdf->HasElement("pitch") ? sdf->Get<double>("pitch") : 0.0;
    yaw_   = sdf->HasElement("yaw") ? sdf->Get<double>("yaw") : 0.0;
  }
  ...
  void CreateStaticTransforms() {
    ...
    transforms.push_back(MakeTransform(timestamp, parent_frame_name_, base_frame_id_, x_, y_, z_, roll_, pitch_, yaw_));
    transforms.push_back(MakeTransform(timestamp, base_frame_id_, depth_camera_frame_id_, 0.0, -0.0115, 0.0, 0.0, 0.0, 0.0));
    transforms.push_back(MakeTransform(timestamp, base_frame_id_, color_camera_frame_id_, 0.0, -0.0115, 0.0, 0.0, 0.0, 0.0));
    transforms.push_back(MakeTransform(timestamp, base_frame_id_, infrared1_camera_frame_id_, 0.0, 0.0175, 0.0, 0.0, 0.0, 0.0));
    transforms.push_back(MakeTransform(timestamp, base_frame_id_, infrared2_camera_frame_id_, 0.0, -0.0325, 0.0, 0.0, 0.0, 0.0));

    // ROS optical frames use x-right, y-down, and z-forward axes.
    transforms.push_back(MakeTransform(timestamp, depth_camera_frame_id_, depth_camera_optical_frame_id_, 0.0, 0.0, 0.0, -kHalfPi, 0.0, -kHalfPi));
    transforms.push_back(MakeTransform(timestamp, color_camera_frame_id_, color_camera_optical_frame_id_, 0.0, 0.0, 0.0, -kHalfPi, 0.0, -kHalfPi));
    transforms.push_back(MakeTransform(timestamp, infrared1_camera_frame_id_, infrared1_camera_optical_frame_id_, 0.0, 0.0, 0.0, -kHalfPi, 0.0, -kHalfPi));
    transforms.push_back(MakeTransform(timestamp, infrared2_camera_frame_id_, infrared2_camera_optical_frame_id_, 0.0, 0.0, 0.0, -kHalfPi, 0.0, -kHalfPi));

    static_tf_broadcaster_->sendTransform(transforms);
  }

  static geometry_msgs::msg::TransformStamped MakeTransform(const rclcpp::Time &timestamp, const std::string &parent_frame, const std::string &child_frame,
  ...
  sensor_msgs::msg::CameraInfo CreateCameraInfo(const sensor_msgs::msg::Image &image, const rendering::CameraPtr camera) const {
    ...
    const double horizontal_focal_length = (image.width / 2.0) / std::tan(horizontal_fov / 2.0);
    const double vertical_fov            = camera->VFOV().Radian();
    const double vertical_focal_length   = (image.height / 2.0) / std::tan(vertical_fov / 2.0);
  ...
  void PublishRosDepthData(const rclcpp::Time &current_time, const rendering::CameraPtr camera) {
    ...
    sensor_msgs::fillImage(depth_image, sensor_msgs::image_encodings::TYPE_16UC1, scaling_ * depth_camera_->ImageHeight(),
                           scaling_ * depth_camera_->ImageWidth(), sizeof(uint16_t) * scaling_ * depth_camera_->ImageWidth(),
                           reinterpret_cast<const void *>(depth_map_.data()));

    const sensor_msgs::msg::CameraInfo camera_info = CreateCameraInfo(depth_image, camera);
  ...
  std::shared_ptr<camera_info_manager::CameraInfoManager> camera_info_manager_{nullptr};
